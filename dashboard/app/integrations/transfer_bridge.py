"""
transfer_bridge.py — HTTP client for HSPSEM_TransferWebApp.gs (see Task 5). The
...
set after HSPSEM_TransferWebApp.gs is deployed (Task 5, Step 3).
"""
...
def form_sync(which: str = "both") -> dict:
    ...
    from HSPSEM_TransferWebApp.gs's doPost({action:'formSync', which})."""
    ...
    if not url or not secret:
        raise FormSyncError(
            ...
            "deploy HSPSEM_TransferWebApp.gs first (see Task 5)."
        )
    resp = requests.post(
